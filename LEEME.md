...
Lo único que hay que hacer en el script es definir la ruta destino donde vamos a 
almacenar las charlas y los subtítulos. Para ello tenemos que editar la línea en 
la que aparece  el parámetro `ttalk_vid_dir` y poner la ruta que deseemos, por 
ejemplo:
...
*Si, lo sé, el separador de directorios en Windows es `\` y no `/`
Es incomodo, pero es necesario sustituir un carácter por otro por como python 
escapa las cadenas, lo siento*
